...
class SatoriResultSsl(SatoriResult):
    protocol: str = "SSL"
    signature: str
...
class SslProcesser(BaseProcesser):
    def __init__(self, xml_path: Optional[str] = None):
        self.xml_path = xml_path
        self.ssl_ja3_xml_exact: Dict[str, List[OsFingerprint]] = defaultdict(list)
        self.ssl_ja3_json_exact: Dict[str, List[OsFingerprint]] = defaultdict(list)
    ...
    def process(self, pkt, layer, ts) -> List[TimedSatoriResult]:
        ...
        for fpType in results:
            ...
            ssl_hash = results[fpType]
            if ssl_hash == "":
                continue
            if fpType == "ja3":
                ssl_fingerprint = ssl_fingerprint_lookup(self.ssl_ja3_xml_exact, ssl_hash)
                ssl_fingerprint.extend(ssl_fingerprint_lookup(self.ssl_ja3_json_exact, ssl_hash))
                ssl_fingerprint.sort(key=lambda item: item.weight)
            elif fpType == "ja3s":
                ssl_fingerprint = ssl_fingerprint_lookup(self.ssl_ja3s_xml_exact, ssl_hash)
            elif fpType == "ja4":
    ...
    def load_fingerprints(self):
        ...
        for x in range(0, len(obj.SSL.fingerprints)):
            ...
            for y in range(0, testsCount):
                ...
                if matchtype == "exact":
                    if testtype == "ja3":
                        self.ssl_ja3_xml_exact[sslsig].append(OsFingerprint(os=os, weight=weight))
                    elif testtype == "ja4":
                        self.ssl_ja4_xml_exact[sslsig].append(OsFingerprint(os=os, weight=weight))
                    elif testtype == "ja3s":
                        self.ssl_ja3s_xml_exact[sslsig].append(OsFingerprint(os=os, weight=weight))
        ...
        if exists(json_file_path):
            try:
                ...
                for fp in contents:
                    self.ssl_ja3_json_exact[fp["md5"]].append(OsFingerprint(os=fp["User-Agent"], weight=weight))
            except Exception as e:
                print(
                    "Issue with ja3er.json, deleting file, redownload again if you want to try again, otherwise just kick off satori the same way again"
                )
                print(e)
        ...
        if exists(file_path):
            try:
                ...
                for fp in jsonObj:
                    self.ssl_ja3_json_exact[fp["ja3_hash"]].append(OsFingerprint(os=fp["desc"], weight=weight))
            except Exception as e:
                print(
                    "Issue with trisulnsm.json, deleting file, redownload again if you want to try again, otherwise just kick off satori the same way again"
                )
                print(e)
                remove(file_path)
                sys.exit(1)
...
def quicProcess(pkt, layer, ts, sslJA4XMLExactList):
    ...
    for fpType in results:
        ...
        if hash != "":
            if fpType == "ja4":
                sslFingerprint = ssl_fingerprint_lookup(sslJA4XMLExactList, hash)
...
def ja3erUpdate():
    ...
    with open(ja3erFile, "wb") as f:
        try:
            ...
            resp = requests.get(
                url, verify=False, timeout=(10, 60)
            )  # not ideal to ignore https, but don't want to deal with certs
            ...
            with open(ja3erGZFile, "wb") as f:
                try:
                    print(
                        "failed to download %s, now attempting to download from %s, this is a compressed version so hopefully faster..."
                        % (url, backupurl)
                    )
                    resp = requests.get(
                        backupurl, verify=False, timeout=(10, 60), allow_redirects=True
                    )  # not ideal to ignore https, but don't want to deal with certs
                    f.write(resp.content)
...
def ssl_fingerprint_lookup(exactList, signature) -> List[OsFingerprint]:
    fingerprint = []
    if signature in exactList:
        fingerprint.extend(exactList.get(signature))
