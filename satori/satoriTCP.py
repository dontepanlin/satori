...
class TcpProcesser(BaseProcesser):
    ...
    def load_fingerprints(self):
        ...
        for x in range(0, len(obj.TCP.fingerprints)):
            os = obj.TCP.fingerprints.fingerprint[x]["name"]
            for y in range(0, len(obj.TCP.fingerprints.fingerprint[x].tcp_tests)):
                ...
                if matchtype == "exact":
                    if tcpflag == "S":
                        self.syn_exact[tcpsig].append(OsFingerprint(os=os, weight=weight))
                    else:  # SA packets
                        self.syn_ack_exact[tcpsig].append(OsFingerprint(os=os, weight=weight))
                else:
                    if tcpflag == "S":
                        self.syn_partial[tcpsig].append(OsFingerprint(os=os, weight=weight))
                    else:  # SA packets
    ...
    def process(self, pkt: Packet):
        ...
        if (winSize != 0) and (mss != 0):
            ...
            p0fSignature = p0fSignature + str(winSize) + ":"
        p0fSignature = (
            p0fSignature + str(ttl) + ":" + str(df) + ":" + str(ipHdrLen + tcpHdrLen) + ":" + tcpOpts + ":" + odd
        )
        ...
        ettercapSignature = (
            etterWinSize + ":" + etterMSS + ":" + hex(ttl).lstrip("0x") + ":" + ws + ":"
        )  # + sack, NOP anywhere, DF, Timestamp Present, Flag of packet (s or a), len

        # build Satori tcp Signature
        # ignore anything that is not S or SA, but should probably clean that up prior to this point!
        tcpSignature = (
            str(winSize) + ":" + str(ttl) + ":" + str(df) + ":" + str(ipHdrLen + tcpHdrLen) + ":" + tcpOpts + ":" + odd
        )
        tcpFingerprint = None
        if tcpFlags == "S":
...
def tcp_fingerprint_lookup(exact, partial, signature):
    ...
    if "*" in signature:
        ...
        newValue3 = (
            splitValue[0] + ":" + splitValue[1] + ":*:" + splitValue[3] + ":" + splitValue[4] + ":" + splitValue[5]
        )

        if newValue1 in partial:
            fingerprint.extend(partial.get(newValue1))
        if newValue2 in partial:
            fingerprint.extend(partial.get(newValue2))
        if newValue3 in partial:
            fingerprint.extend(partial.get(newValue3))
