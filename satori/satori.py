...
try:
    ...
    if args.filter != "":
        if args.directory != "":
            print(
                "Filter not implemented in directory processing, please remove and try again",
                end="\n",
                flush=True,
            )
            sys.exit(1)
        if args.readpcap != "":
            print(
                "Filter not implemented in pcap file read processing, please remove and try again",
                end="\n",
                flush=True,
            )
            sys.exit(1)
